  ...
  const genrateSummary = async () => {
    try {
      const res = await axios.post(
        ...
        }/profile/generateSummary?resume=${user.profile.resume}`,
        { withCredentials: true }
      );
      console.log("summary is : ", res.data);
    } catch (err) {
      console.log("error in genratign summary");
    }
  };
        ...
        <div className="max-w-4xl mx-auto bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl border border-white/20 dark:border-slate-700/50 rounded-3xl shadow-2xl my-8 p-8 md:p-12">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 mb-4">
              ...
              <Label className="text-xl font-semibold text-slate-800 dark:text-white">
                Resume
              </Label>
            </div>
          ...
          {ishaveResume && (
            <div>
              profile Description :
              <button
                className="text-sm bg-blue-600 p-2"
